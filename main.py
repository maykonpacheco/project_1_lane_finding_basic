...
if __name__ == '__main__':
    ...
    for test_img in test_images:
        ...
        in_image = cv2.cvtColor(cv2.imread(
            test_img, cv2.IMREAD_COLOR), cv2.COLOR_BGR2RGB)
    ...
    test_videos = [join(test_videos_dir, name)
    ...
    for test_video in test_videos:
        ...
        while cap.isOpened():
            ...
            if ret:
                ...
                blend_frame = color_frame_pipeline(
                    frames=frame_buffer, solid_lines=True, temporal_smoothing=True)
                out.write(cv2.cvtColor(blend_frame, cv2.COLOR_RGB2BGR))
                cv2.imshow('blend', cv2.cvtColor(
                    blend_frame, cv2.COLOR_RGB2BGR)), cv2.waitKey(1)
            else:
                break
        cap.release()
        out.release()
